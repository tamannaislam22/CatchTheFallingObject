...
public class GamePanel extends JPanel implements KeyListener {
    ...
    // Week 5: Catch effect
    private boolean[] showCatchEffect = new boolean[numberOfObjects];
    private int[] effectX = new int[numberOfObjects];
    private int[] effectY = new int[numberOfObjects];
        ...
        if (showMessage) {
            ...
            for (int i = 0; i < numberOfObjects; i++) {
                ...
                if (showCatchEffect[i]) {
                    g.setColor(Color.WHITE);
                    g.setFont(new Font("Arial", Font.BOLD, 20));
                    g.drawString("+1", effectX[i], effectY[i]);
                }
            }
        }
    }
